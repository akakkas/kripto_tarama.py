...
INTERVAL="30m"; PERIOD=10; MULT=3.0
...
def sim_5(entry_ms, yon, entry, stop, TS5, H5, L5, C5, MB5, skip=0):
    """5m orta bant kırılımında çık. skip=kaç kırılış yutulacak (retest payı).
    Stop her an aktif (sert taban). Döner (exit_px, bars5) veya None."""
    j0=int(np.searchsorted(TS5, entry_ms))
    if j0>=len(C5)-2: return None
    last=min(len(C5)-1, j0+MAXBARS_5)
    prev_below=False; breaks=0
    for j in range(j0, last+1):
        hi=H5[j]; lo=L5[j]; cl=C5[j]; mb=MB5[j]
        if (lo<=stop) if yon=="AL" else (hi>=stop):
            return (stop, j-j0)
        if not np.isnan(mb):
            below = (cl<mb) if yon=="AL" else (cl>mb)   # orta bandın ters tarafında mı
            if below and not prev_below:                 # yeni bir kırılış olayı
                breaks+=1
                if breaks>skip: return (cl, j-j0)
            prev_below=below
...
def run():
    ...
    print(f"Evren: {len(syms)} coin · {DAYS} gün · giriş 30m / çıkış 5m\n")

    trades=[]; need=max(PERIOD,BBP)+LOOKBACK+3
    for idx,sym in enumerate(syms,1):
        d15=fetch_tf(ex,sym,DAYS,"30m",30*60*1000)
        ...
        for i in range(need, len(C)-1):
            ...
            entry_ms=int(TS[i])+30*60*1000
            ex5b=sim_5(entry_ms,yon,px,stop,TS5,H5,L5,C5,MB5,skip=0)   # ilk kırılışta çık
            ex5c=sim_5(entry_ms,yon,px,stop,TS5,H5,L5,C5,MB5,skip=1)   # ilk kırılışı yut, 2.de çık
            if ex5b is None or ex5c is None: continue
            px_b,bars5=ex5b; px_c2,_=ex5c
            px_a,bar_a=r15["A"]
            ...
            row["R_C"]=round(Rv(yon,px,px_c2,risk),4); row["pct_C"]=round(pct(yon,px,px_c2),3)
            trades.append(row); cs+=1
            # kilidi referans modele (A=30m BAND, en kısa tutan) bağla
            open_until=i+bar_a
        print(f"[{idx}/{len(syms)}] {sym}: {cs} giriş")

    if not trades: print("\nHiç giriş yok."); return
    out=os.path.expanduser("~/bt_30m_trades.csv")
    ...
    MODELS=[("A","30m BAND (vurkaç)"),("B","5m MID (ilk kırılışta çık)"),("C","5m MID skip-1 (retest yut)")]
    print("\n"+"="*80)
    print(f"30m GİRİŞ / 5dk ÇIKIŞ — {len(trades)} giriş · {DAYS}g · {len(syms)} coin · komisyon={FEE*100:.3f}%/yön")
    print("="*80)
